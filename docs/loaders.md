...
### Дубли между vanilla и loader на обычном classpath (без module path вообще)

Ещё один вариант того же класса проблем, но без `-p` в принципе - у **Fabric** его нет вообще
(плоский classpath, никакого module path). Vanilla version json для ряда версий (например
1.21.4) сам объявляет `org.ow2.asm:asm:9.6` (то же самое пиновое использование ASM Mojang'ом,
что и раньше на 1.21.6), а Fabric Loader для той же версии несёт `org.ow2.asm:asm:9.10.1` -
разные точные координаты (имя с версией), значит дедуп по `lib.get("name")` их не ловит, и обе
версии одного и того же класса `org/objectweb/asm/...` оказываются на `-cp`. Тут падает уже не
JVM module system, а **сам Fabric Loader** - `LoaderUtil.verifyClasspath` при старте детектит
дубликат класса на classpath и явно ругается.

Раньше `_module_path_artifact_keys`/`_artifact_key`-дедуп по `group/artifact` применялся только
к тому, что явно упомянуто в `-p` - для Fabric `-p` не существует, так что фильтр не работал
вообще. Фикс - тот же дедуп по `group/artifact` (не по версии) теперь применяется **всегда**,
а не только для `-p`-совпадений: если vanilla и loader оба объявляют один и тот же артефакт (в
любой версии), версия загрузчика побеждает безусловно, версия vanilla выбрасывается из
`merged["libraries"]`. Опирается на то, что ASM (и подобные библиотеки) обратно совместимы -
новая версия почти всегда умеет всё, что умела старая.
